...
class Key extends Authentication {
    ...
    /**
     * @typedef {object} GetUserClientConfigBufferResponse
     ...
     * @param {string} organisationId ID of the organisation containing the target user.
     * @param {string} userId ID of the user to fetch config(s) for.
     * @param {object} [options] See parameter `options` of `getUserClientConfigStream()`. 
     * @returns {Promise<GetUserClientConfigBufferResponse>}
     */
    async getUserClientConfigBuffer(organisationId, userId, options) {
        const {
            stream,
            fileName,
            fileSize,
        } = await this.getUserClientConfigStream(organisationId, userId, options);
        ...
        // Wait for our stream to either error out or end nicely...
        await Promise.race([
