...
class User extends Authentication {
    ...
    async updateUser(organisationId, userId, userPatchData) {
        // We've got to fetch back the full user object to actually build our update since
        // this route will replace the target user. For this reason it's very important to
        // be careful about race hazards from consumers; try to limit or otherwise safely
        // schedule updates done in quick succession.
        const userObject = await this.getUserById(organisationId, userId);
        
        if (!(userObject instanceof Object)) {
            throw new Error(`Failed to resolve target user data from the given ID: ${userId}`);
        };

        const res = await super.authorizedRequest({
            method: 'PUT',  // If only `PATCH` were a thing.
            path: `/user/${organisationId}/${userObject.id}`,
            data: {
                ...userObject,
                ...userPatchData,
            },
